...
import { Package, Search, Filter, Plus, Download, BarChart3, MessageSquare, ScrollText } from 'lucide-react';
...
const InventoryPage: React.FC = () => {
  ...
  const tabs = React.useMemo(() => ['inventory', 'update', 'commands', 'logs'], []);
    ...
    try {
      if (!isAuthenticated) {
        toast({
          ...
          description: "Configure Google Sheets primeiro",
          variant: "destructive",
        });
        return;
      }

      // Implementar exportação para Google Sheets
      toast({
        title: "Em desenvolvimento",
        description: "Exportação para Google Sheets em breve",
      });
    } catch (error) {
      toast({
        title: "Erro",
        description: "Falha ao exportar para Google Sheets",
        variant: "destructive",
      });
    }
  };
  ...
  const handleUpdateItemsInAllSheets = React.useCallback((items: Array<{ name: string; quantity: number }>, operation: 'add' | 'set' = 'add') => {
    ...
    items.forEach(({ name, quantity }) => {
      sheets.forEach((sheet) => {
        const itemExists = sheet.items.some(item => 
        ...
        if (itemExists || !itemExists) {
          totalProcessed++;
          if (!updatedSheets.includes(sheet.name)) {
    ...
    toast({
      ...
      description: `${items.length} item(s) processado(s) em ${updatedSheets.length} planilha(s)`,
    });
  }, [sheets, updateItem]);
  ...
  const handleUpdateItemsInAllSheets = React.useCallback((items: Array<{ name: string; quantity: number }>, operation: "add" | "set" = "add") => {
    ...
    toast({
      title: `Itens ${operationText} em todas as abas`,
      description: `${items.length} item(s) processado(s) em ${sheets.length} planilha(s)`,
    });
  }, [sheets]);
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="update">
            ...
            >
                              <UpdateForm
                  onSubmit={(data) => {
                    updateItem(data.name, data.quantity, 'add');
                  }}
                />
            </motion.div>
          </TabsContent>
